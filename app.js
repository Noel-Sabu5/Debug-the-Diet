...
(function () {
  ...
  ───────────────────────────────────────────── */
  const GEMINI_API_KEY = 'AIzaSyD89tphO-loIf8nUvFu1uLe1eOUo4Q6oIY';
  const GEMINI_ENDPOINT =
    'https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=' + GEMINI_API_KEY;

  async function getAIInsights(meal) {
    const prompt =
      `Analyze this meal and give health issues and improvement suggestions in short bullet points. Meal: "${meal}". ` +
      `Respond with ONLY 3–4 concise bullet points (no headers, no markdown bold, just plain text lines starting with "•").`;
