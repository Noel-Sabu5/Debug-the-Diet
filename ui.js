...
function updateInsightsPage(stats) {
  ...
  if (trendIcon) {
    trendIcon.textContent = stats.trend >= 0 ? 'trending_up' : 'trending_down';
  }

  // Total meals
  const totalEl = document.getElementById('total-meals');
  if (totalEl) totalEl.textContent = stats.totalMeals;

  // Days in target (estimate from weeklyMeals with score >= 70)
  const daysInRange = (stats.weeklyMeals || []).filter(m => m.score >= 70).length;
  const daysEl = document.getElementById('days-in-range');
  if (daysEl) daysEl.textContent = `${Math.min(daysInRange, 7)} out of 7 days within target range`;

  // Primary deficit
  const deficitEl = document.getElementById('primary-deficit');
  if (deficitEl && stats.weeklyMeals && stats.weeklyMeals.length > 0) {
...
/**
 * Update the curated summary section.
 */
function updateCuratedSummary(stats) {
  ...
  const trendText = trend > 0
    ? `improving by <span class="text-primary dark:text-primary-fixed font-semibold">+${trend}%</span> vs last week`
    : trend < 0
    ? `declining by <span class="text-tertiary dark:text-tertiary-fixed font-semibold">${trend}%</span> vs last week`
